...
class RangeSelectionSliderTrackLayer: CALayer {
    ...
    override func draw(in contex: CGContext) {
        guard let slider = rangeSlider else{
            ...
            let rect = CGRect(x: lowerValPosition, y: 0.0, width: upperValPosition - lowerValPosition, height: bounds.height)
            contex.fill(rect)
...
/**
 This class draws rectangle[In the form of circle or square] for track indicator layer on a bezier path and fills color to it. curvaceousness defines the cornerradius for indicator layer. And changes indicator color when it is highlighted.
 */
...
/**
 This class combines track layer and indicators layer to form a selection slider for selecting a range. Defines different properties to change the UI dynamically.
 */
...
@IBDesignable public class InnoRangeSelectionSlider: UIControl {
   ...
   fileprivate var indicatorWidth: CGFloat {
    ...
    func updateRangeSliderLayerFrames() {
        ...
        lowerIndicatorLayer.frame = CGRect(x: lowerThumbCenter - indicatorWidth / 2.0, y: 0.0,width: indicatorWidth, height: indicatorWidth)
        lowerIndicatorLayer.setNeedsDisplay()
        
        let upperThumbCenter = CGFloat(positionForValue(value: upperValue))
        upperIndicatorLayer.frame = CGRect(x: upperThumbCenter - indicatorWidth / 2.0, y: 0.0,width: indicatorWidth, height: indicatorWidth)
        upperIndicatorLayer.setNeedsDisplay()
    ...
    override public func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        if lowerIndicatorLayer.highlighted {
            ...
            lowerValue = boundValue(value: lowerValue, toLowerValue: minValue, upperValue: upperValue - gapBetweenIndicators)
        } else if upperIndicatorLayer.highlighted {
            upperValue += deltaValue
            upperValue = boundValue(value: upperValue, toLowerValue: lowerValue+gapBetweenIndicators, upperValue:maxValue)
        }
